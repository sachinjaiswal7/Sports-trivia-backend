...
router.route("/selectedQuestions")
    ...
    .post(isAuthenticated, async (req, res, next) => {
        ...
        try {
            ...
            // if the half is first then we create a new entry in the userSelectedQuestions else we update the userSelectedQuestions if it is for more than first half.
            if (half == 1) {
                ...
                alreadySelectedQuestions.choosenQuestion.push(obj);
    ...
    try {
        ...
        for (let i = 0; i < matchQuestions.length; i++) {
            ...
            for (let j = 0; j < matchQuestions[i]?.twoPointer?.length; j++) {
                if (mySelectedQuestions?.choosenQuestion?.length >= (half + 1)) {
